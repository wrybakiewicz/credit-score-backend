...
describe('test FollowersFollowingsScore', function () {
    ...
    it('should return details', async () => {
        ...
        expect(details.followingCount).to.be.equal(2);
        expect(details.friends.length).to.be.equal(1);
        expect(details.twitterName).to.be.equal("guziec_pl");
        expect(details.friends[0]).to.be.equal("0x12cd21d7dacc71c631f8e645240d80ad560f0161");
    });
    ...
    it('should return details - for address without friends & twitter', async () => {
        ...
        expect(details.followerCount).to.be.equal(0);
        expect(details.followingCount).to.be.equal(0);
        expect(details.friends.length).to.be.equal(0);
